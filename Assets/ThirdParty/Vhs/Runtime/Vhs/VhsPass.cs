...
namespace VolFx
{
    ...
    public class VhsPass : VolFx.Pass
    {
		...
		private                 bool  _isVolFxOn;
		private static readonly int   s_InputC = Shader.PropertyToID("_InputC");
		...
		public override bool Validate(Material mat)
        {
			...
			if (_isVolFxOn == false)
			{
				mat.EnableKeyword("_STANDALONE_SCANLINES_ON");
				_isVolFxOn = true;
			}
#endif
			...
			if (_isVolFxOn)
			{
			    var _Sl_Count      = Mathf.Lerp(50f, 800f, settings._sl_count.value);
			    var _Sl_Intensity  = settings._sl_intensity.value * 1.5f;
				var _Sl_Move       = Time.time * settings._sl_move.value * _Sl_Count * .2f;
				var _Sl_Flickering = Mathf.SmoothStep(0f, 1f, (Time.time * 1.5f) % 1f);

			    mat.SetVector(s_InputC, new Vector4(_Sl_Count, _Sl_Intensity, _Sl_Move, _Sl_Flickering));
			}
#endif
