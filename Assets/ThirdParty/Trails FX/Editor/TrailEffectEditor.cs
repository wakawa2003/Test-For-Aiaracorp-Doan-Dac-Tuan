...
namespace TrailsFX {
    ...
    public class TrailEffectEditor : Editor {
        ...
        SerializedProperty useLastAnimationState, maxBatches, meshPoolSize, interpolate, animationStates, animationStateNormalizedStart, animationStateNormalizedEnd, animator, preserveMultiMaterials;
        ...
        void OnEnable () {
            ...
            animationStateNormalizedStart = serializedObject.FindProperty("animationStateNormalizedStart");
            animationStateNormalizedEnd = serializedObject.FindProperty("animationStateNormalizedEnd");
        ...
        public override void OnInspectorGUI () {
            ...
            if (thisEffect.isParticle) {
                ...
                if (animationStates.stringValue != null && animationStates.stringValue.Length > 0) {
                    float winStart = animationStateNormalizedStart.floatValue;
                    float winEnd = animationStateNormalizedEnd.floatValue;
                    EditorGUI.BeginChangeCheck();
                    EditorGUI.indentLevel++;
                    Rect totalRect = EditorGUILayout.GetControlRect();
                    GUIContent rangeLabel = new GUIContent("Progress Range", "Trail is active while the filtered state's playback progress is inside this [start, end] range (0 = state start, 1 = state end). Handles looping states via Mathf.Repeat.");
                    Rect fieldsRect = EditorGUI.PrefixLabel(totalRect, rangeLabel);
                    int savedIndent = EditorGUI.indentLevel;
                    EditorGUI.indentLevel = 0;
                    float fieldW = 54f;
                    float gap = 6f;
                    Rect startRect = new Rect(fieldsRect.x, fieldsRect.y, fieldW, fieldsRect.height);
                    Rect sliderRect = new Rect(fieldsRect.x + fieldW + gap, fieldsRect.y, fieldsRect.width - (fieldW + gap) * 2, fieldsRect.height);
                    Rect endRect = new Rect(fieldsRect.xMax - fieldW, fieldsRect.y, fieldW, fieldsRect.height);
                    winStart = EditorGUI.FloatField(startRect, winStart);
                    EditorGUI.MinMaxSlider(sliderRect, ref winStart, ref winEnd, 0f, 1f);
                    winEnd = EditorGUI.FloatField(endRect, winEnd);
                    EditorGUI.indentLevel = savedIndent;
                    EditorGUI.indentLevel--;
                    if (EditorGUI.EndChangeCheck()) {
                        winStart = Mathf.Clamp01(winStart);
                        winEnd = Mathf.Clamp01(winEnd);
                        if (winEnd < winStart) winEnd = winStart;
                        animationStateNormalizedStart.floatValue = winStart;
                        animationStateNormalizedEnd.floatValue = winEnd;
                    }
                }
