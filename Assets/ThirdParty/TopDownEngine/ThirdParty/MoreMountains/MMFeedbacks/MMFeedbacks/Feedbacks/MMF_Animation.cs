...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Animation : MMF_Feedback 
	{
		...
		public enum ValueModes { None, Constant, Random, Incremental, Circulation }
		...
		/// the min value for int circulation mode
		[Tooltip("the min value for int circulation mode")]
		[MMFEnumCondition("IntValueMode", (int)ValueModes.Circulation)]
		public int IntCirculationMin = 0;
		/// the max value for int circulation mode
		[Tooltip("the max value for int circulation mode")]
		[MMFEnumCondition("IntValueMode", (int)ValueModes.Circulation)]
		public int IntCirculationMax = 10;
		/// the increment value for int circulation mode
		[Tooltip("the increment value for int circulation mode")]
		[MMFEnumCondition("IntValueMode", (int)ValueModes.Circulation)]
		public int IntCirculationIncrement = 1;
		...
		/// the min value for float circulation mode
		[Tooltip("the min value for float circulation mode")]
		[MMFEnumCondition("FloatValueMode", (int)ValueModes.Circulation)]
		public float FloatCirculationMin = 0f;
		/// the max value for float circulation mode
		[Tooltip("the max value for float circulation mode")]
		[MMFEnumCondition("FloatValueMode", (int)ValueModes.Circulation)]
		public float FloatCirculationMax = 10f;
		/// the increment value for float circulation mode
		[Tooltip("the increment value for float circulation mode")]
		[MMFEnumCondition("FloatValueMode", (int)ValueModes.Circulation)]
		public float FloatCirculationIncrement = 1f;
		...
		protected virtual void ApplyValue(Animator targetAnimator, float intensityMultiplier)
		{
			...
			switch (IntValueMode)
			{
				...
				case ValueModes.Random:
					...
					break;
				case ValueModes.Circulation:
					int currentIntValue = targetAnimator.GetInteger(_intParameter);
					if (currentIntValue >= IntCirculationMax)
					{
						currentIntValue = IntCirculationMin;
					}
					else if (currentIntValue < IntCirculationMin)
					{
						currentIntValue = IntCirculationMax;
					}
					else
					{
						currentIntValue += IntCirculationIncrement;
					}
					targetAnimator.SetInteger(_intParameter, currentIntValue);
					break;
			}
			...
			switch (FloatValueMode)
			{
				...
				case ValueModes.Random:
					...
					break;
				case ValueModes.Circulation:
					float currentFloatValue = targetAnimator.GetFloat(_floatParameter);
					if (currentFloatValue >= FloatCirculationMax)
					{
						currentFloatValue = FloatCirculationMin;
					}
					else if (currentFloatValue < FloatCirculationMin)
					{
						currentFloatValue = FloatCirculationMax;
					}
					else
					{
						currentFloatValue += FloatCirculationIncrement * intensityMultiplier;
					}
					targetAnimator.SetFloat(_floatParameter, currentFloatValue);
					break;
			}
