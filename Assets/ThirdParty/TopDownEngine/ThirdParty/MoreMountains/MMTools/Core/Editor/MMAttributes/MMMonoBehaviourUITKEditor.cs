...
namespace MoreMountains.Tools
{
	...
	public class MMMonoBehaviourUITKEditor : Editor
	{
		...
		{
			// [로컬 패치 2026-07-22] 플레이 중 인스펙터에 선택된 오브젝트(예: 죽은 적의 GuardableHealth)가
			// 파괴되면 target.name 접근에서 MissingReferenceException 발생 → 방어적으로 조기 반환.
			...
			foldout.viewDataKey = (target != null ? target.name : "<destroyed>") + "-" + _targetTypeName + groupData.GroupAttribute.GroupName;
			root.Add(foldout);
