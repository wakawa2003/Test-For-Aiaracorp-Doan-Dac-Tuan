    ...
    string[] colorProperties =
   {
    ...
    void UpdateAlphaByProperties(string rendName, int materialNumber, Material mat, float alpha)
    {
        foreach (var prop in colorProperties)
        {
            ...
            {
                if (!startAlphaColors.ContainsKey(rendName + materialNumber + prop.ToString()))
                    GetStartAlphaByProperties(rendName, materialNumber, mat);
        ...
        foreach (var prop in colorProperties)
        {
            if (mat.HasProperty(prop))
            {
                ...
                if (!startAlphaColors.ContainsKey(key))                
                    startAlphaColors.Add(rendName + materialNumber + prop.ToString(), 1);
            }
        }
    }
